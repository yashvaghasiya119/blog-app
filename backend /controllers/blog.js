...
const addBlog = async (req, res) => {
  ...
  try {
    ...
    // Basic validations
    if (!title || title.trim().length < 3) {
      return res.status(400).json({ err: true, error: "Title is required and must be at least 3 characters." });
    }

    if (!body || body.trim().length === 0) {
      return res.status(400).json({ err: true, error: "Body content is required." });
    }

    if (!req.user || !req.user.id) {
      return res.status(401).json({ err: true, error: "Unauthorized. User ID not found in token." });
    }

    if (!file) {
      return res.status(400).json({ err: true, error: "No image uploaded." });
    }

    const maxSize = 1 * 1024 * 1024; // 1MB
    if (file.size > maxSize) {
      return res.status(400).json({ err: true, error: "Image is too large. Max allowed size is 1MB." });
    }
    ...
    console.error("Error adding blog:", error);
    return res.status(500).json({ err: true, error: "Internal server error" });
  }
};
...
const updateBlog = async (req, res) => {
  try {
    ...
    if (!blog) {
      return res.status(404).json({ err: true, message: 'Blog not found' });
    }

    // Check ownership
    if (blog.createdBy.toString() !== userId) {
      return res.status(403).json({ err: true, message: 'Unauthorized to update this blog' });
    }
    ...
    console.error('Update blog error:', error.message);
    res.status(500).json({ err: true, message: 'Failed to update blog' });
  }
};
...
const getSingleBlog = async (req, res) => {
  try {
    ...
    if (!id.match(/^[0-9a-fA-F]{24}$/)) {
      return res.status(400).json({ err: true, success: false, message: "Invalid blog ID format" });
    }

    const blog = await Blog.findById(id);

    if (!blog) {
      return res.status(404).json({ err: true, success: false, message: "Blog not found" });
    }

    res.status(200).json({ err: false, success: true, data: blog });
  } catch (error) {
    console.error("Error fetching blog:", error);
    res.status(500).json({ err: true, success: false, message: "Server error" });
  }
};
